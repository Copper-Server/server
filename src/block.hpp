...
namespace mcCore {
	...
	public:
		...
		block_id_t id;
		Block(block_id_t block_id, const ENBT& enbt) :id(block_id) {
			if (!can_has_nbt())
				throw std::invalid_argument("this block not hold nbt");
			nbt["nbt"] = enbt;
		}
		Block(block_id_t block_id, ENBT&& enbt) :id(block_id) {
			if (!can_has_nbt())
				throw std::invalid_argument("this block not hold nbt");
			nbt["nbt"] = std::move(enbt);
		}
		Block(block_id_t block_id = 0) :id(block_id) { nbt = nullptr; }
		...
		struct TickAnswer {
			...
			TickAnswer(Behavior beh, Block bl, PosMode pm = PosMode::as_tunels, uint8_t xbs = 0, uint8_t ybs = 0, uint8_t zbs = 0, uint8_t Xbs = 0, uint8_t Ybs = 0, uint8_t Zbs = 0, uint8_t mx = 0, uint8_t my = 0, uint8_t mz = 0) {
				behavior = beh;
				mode = pm;
				x = xbs;
				y = ybs;
	...
	struct BlockHash
	{
		std::size_t operator()(const Block& k) const
		{
			...
			return ((hash<block_id_t>()(k.id)
				^ (hash<const void*>()(&k.getStaticData()) << 2)) >> 2)
				^ ((hash<const void*>()(&k.get_nbt()) << 1)>>1)
				^ (hash<const void*>()(&k.get_state()) << 1);
		}
	};
}
#pragma pack(pop)