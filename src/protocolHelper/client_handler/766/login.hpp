        ...
        namespace release_766 {
            ...
            protected:
                ...
                Response encryptionRequest() {
                    ...
                    return packets::release_766::login::encryptionRequest("", verify_token);
                }

                Response loginSuccess() {
                    auto tmp = packets::release_766::login::loginSuccess(session->sharedData());
                ...
                Response setCompression(int32_t threshold) {
                    Response response = packets::release_766::login::setCompression(threshold);
                ...
                Response proceedPlugin(ArrayStream& data, bool successful = true) {
                    ...
                    while (plugins_query.size()) {
                        ...
                        if (std::holds_alternative<PluginRegistration::PluginResponse>(response)) {
                            ...
                            plugin_message_id++;
                            return packets::release_766::login::login(plugin_message_id, plugin.plugin_chanel, plugin.data);
                    ...
                    if (
                        ...
                        api::players::iterate_players_not_state(base_objects::SharedClientData::packets_state_t::protocol_state::initialization, [&](base_objects::SharedClientData& player) {
                            if (player.name == session->sharedData().name) {
                                ...
                                case ps::play:
                                    player.sendPacket(packets::release_766::play::kick(reason));
                                    break;
                                case ps::configuration:
                                    player.sendPacket(packets::release_766::configuration::kick(reason));
                                default:
                                    break;
                                }
                            }
                ...
                Response WorkPacket(ArrayStream& packet) override {
                    ...
                    case 0: { //login start
                        ...
                        if (kick_reason_chat) {
                            ...
                            return packets::release_766::login::kick(str);
                        }
                        if (
                    ...
                    case 1: { //encryption response
                        ...
                        if (!Server::instance().decrypt_data(verify_token)) {
                            ...
                            return packets::release_766::login::kick("Invalid verify token");
                        }
                        ...
                        if (!Server::instance().decrypt_data(shared_secret)) {
                            ...
                            return packets::release_766::login::kick("Encryption error");
                        }
                    ...
                    case 4: { //from 766 Cookie response
                ...
                Response TooLargePacket() override {
                    return packets::release_766::login::kick("Packet too large");
                }

                Response Exception(const std::exception& ex) override {
                    return packets::release_766::login::kick("Caught exception: " + std::string(ex.what()));
                }

                Response UnexpectedException() override {
                    return packets::release_766::login::kick("Caught unexpected exception");
                }
...
#endif /* SRC_PROTOCOLHELPER_CLIENT_HANDLER_766_LOGIN */
