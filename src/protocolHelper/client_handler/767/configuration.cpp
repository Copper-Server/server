...
namespace copper_server::client_handler::release_767 {
    ...
    base_objects::network::response handle_configuration::work_packet(ArrayStream& packet) {
        ...
        case 0x07: {
            ...
            for (int32_t i = 0; i < len; i++) {
                std::string name = packet.read_string(32767);
                std::string id = packet.read_string(32767);
                std::string version = packet.read_string(32767);
    ...
    void handle_configuration::queryPacket(const PluginRegistration::PluginResponse& packet) {
        queriedPackets.push_back(
            packets::release_767::configuration::configuration(packet.plugin_chanel, packet.data)
        );
    }
}