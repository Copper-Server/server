        ...
        namespace release_767 {
            ...
            protected:
                Response IdleActions() {
                    ...
                    for (auto& it : load_next_packets) {
                        if (std::holds_alternative<PluginRegistration::PluginResponse>(it)) {
                            ...
                            response += packets::release_767::configuration::configuration(plugin.plugin_chanel, plugin.data);
                ...
                Response FinishConfiguration() {
                    ...
                    return packets::release_767::configuration::finish();
                }

                Response Ping() {
                    log::debug("configuration", "Ping");
                    pong_timer = std::chrono::system_clock::now();
                    return packets::release_767::configuration::ping(excepted_pong = generate_random_int());
                }

                Response RegistryData() {
                    log::debug("configuration", "Registry data");
                    return packets::release_767::configuration::registry_data();
                }
                ...
                Response WorkPacket(ArrayStream& packet) override {
                    ...
                    case 0x02: { //plugin message
                        ...
                        if (it != nullptr) {
                            ...
                            if (std::holds_alternative<PluginRegistration::PluginResponse>(result)) {
                                ...
                                log::debug("configuration", "Plugin message");
                                return packets::release_767::configuration::configuration(plugin.plugin_chanel, plugin.data);
                    ...
                    case 0x03: //configuration complete
                        if (!session->sharedData().packets_state.pending_resource_packs.empty())
                            return packets::release_767::configuration::kick("You are not downloaded all requested packs.");
                    ...
                    case 0x07: {
                        ...
                        for (int32_t i = 0; i < len; i++) {
                            ...
                            std::string id = ReadString(packet, 32767);
                            std::string version = ReadString(packet, 32767);
                ...
                Response Exception(const std::exception& ex) override {
                    return packets::release_767::configuration::kick(ex.what());
                }

                Response UnexpectedException() override {
                    return packets::release_767::configuration::kick("Unexpected exception");
                }
            ...
            public:
                HandleConfiguration(TCPsession* sock)
                    ...
                    keep_alive_solution->set_callback([this]() {
                        ...
                        return packets::release_767::configuration::keep_alive(keep_alive_packet);
                    });
