...
namespace copper_server::packets::release_768::reader {
    namespace slot_component_encoder {
        ...
        void encode(const slot_data& slot, list_array<uint8_t>& data, const slot_component::attribute_modifiers& value) {
            ...
            for (auto& attribute : value.attributes) {
                try {
                    WriteVar<int32_t>(registers::view_reg_pro_id("minecraft:attribute", attribute.type, 768), data);
                } catch (...) {
                ...
                [&data](auto& it) {
                    if constexpr (std::is_same_v<std::decay_t<decltype(it)>, std::string>) {
                        ...
                        try {
                            ...
                            id = registers::view_reg_pro_id("minecraft:sound_event", "minecraft:item.armor.equip_generic", 768);
                        }
                        WriteVar<int32_t>(1 + id, data);
                    } else if constexpr (std::is_same_v<std::decay_t<decltype(it)>, std::nullptr_t>) {
                        WriteVar<int32_t>(1 + registers::view_reg_pro_id("minecraft:sound_event", "minecraft:item.armor.equip_generic", 768), data);
                    } else {
                        WriteVar<int32_t>(0, data); //custom
                        WriteIdentifier(data, it.sound_name);
                        data.push_back((bool)it.fixed_range);
                    ...
                    [&data](auto& it) {
                        if constexpr (std::is_same_v<std::decay_t<decltype(it)>, std::string>) {
                            ...
                            for (auto& i : it) {
                                int32_t id = 0;
                                try {
                                    id = registers::view_reg_pro_id("minecraft:entity_type", i, 768);
                                } catch (...) {
                                    id = registers::view_reg_pro_id("minecraft:entity_type", "minecraft:player", 768);
                                }
                                WriteVar<int32_t>(id, data);
                            }
                        }
            ...
            std::visit(
                [&data](auto& it) {
                    if constexpr (std::is_same_v<std::decay_t<decltype(it)>, std::string>) {
                        ...
                        WriteVar<int32_t>(1 + it.size(), data); //type 1 + arr size 1
                        for (auto& i : it) {
                            int32_t id = slot_data::get_slot_data(i).internal_item_aliases.at(768).local_id;
                            WriteVar<int32_t>(id, data);
                        }
                    }
                },
    ...
    namespace slot_component_decoder {
        ...
        list_array<std::string> variants(int32_t id) {
            return {
                registers::view_reg_pro_name("minecraft:data_component_type", id, 768)
            };
        }
    ...
    void WriteIngredient(list_array<uint8_t>& data, const base_objects::slot_display& item) {
        std::visit(
            [&](auto&& arg) {
                ...
                WriteVar<int32_t>(registers::view_reg_pro_id("minecraft:recipe_display", T::name, 768), data);
                if constexpr (
                    ...
                    WriteVar<int32_t>(registers::view_reg_pro_id("minecraft:item", arg.item, 768), data);
                } else if constexpr (std::is_same_v<T, base_objects::slot_displays::minecraft::item_stack>) {
                    WriteSlotItem(data, arg.item, 768);
            ...
            [&data](auto& it) {
                ...
                if constexpr (T_info::must_display) {
                    if constexpr (std::is_same_v<T, base_objects::recipes::minecraft::crafting_shapeless>) {
                        WriteVar<int32_t>(registers::view_reg_pro_id("minecraft:recipe_display", T_info::name, 768), data);
                        ...
                        WriteVar<int32_t>(registers::view_reg_pro_id("minecraft:recipe_display", "minecraft:furnace", 768), data);
                        ...
                        WriteVar<int32_t>(registers::view_reg_pro_id("minecraft:recipe_display", "minecraft:stonecutter", 768), data);
                        WriteIngredient(data, it.ingredient);
                        WriteIngredient(data, it.result);
                        WriteIngredient(data, it.crafting_station);
                    } else if constexpr (std::is_same_v<T, base_objects::recipes::minecraft::smithing_transform>) {
                        WriteVar<int32_t>(registers::view_reg_pro_id("minecraft:recipe_display", "minecraft:smithing", 768), data);
