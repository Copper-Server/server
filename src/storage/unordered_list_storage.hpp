#ifndef SRC_STORAGE_UNORDERED_LIST_STORAGE
#define SRC_STORAGE_UNORDERED_LIST_STORAGE
#include <filesystem>
...
namespace copper_server::storage {
    class unordered_list_storage {
        fast_task::protected_value<std::unordered_set<std::string>> data;
        std::filesystem::path path;
        bool _is_loaded = false;

    public:
        unordered_list_storage(const std::filesystem::path& path);

        void add(const std::string& value);
        bool contains(const std::string& value);
        void remove(const std::string& value);

        std::unordered_set<std::string> entrys(size_t max_items, bool& max_reached);
...
#endif /* SRC_STORAGE_UNORDERED_LIST_STORAGE */
