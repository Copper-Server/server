/*
 * Copyright 2024-Present Danyil Melnytskyi. All Rights Reserved.
 *
 * Licensed under the Apache License 2.0 (the "License"). You may not use
 * this file except in compliance with the License. You can obtain a copy
 * in the file LICENSE in the source distribution or at
 * http://www.apache.org/licenses/LICENSE-2.0
 */
#include <library/enbt/senbt.hpp>
#include <library/fast_task/include/files.hpp>
#include <src/api/configuration.hpp>
#include <src/api/id.hpp>
#include <src/api/internal/world.hpp>
#include <src/api/log.hpp>
#include <src/api/packets/client_bound/play.hpp>
#include <src/api/players.hpp>
#include <src/api/world.hpp>
#include <src/base_objects/commands.hpp>
#include <src/base_objects/player.hpp>
#include <src/plugin/main.hpp>
#include <src/storage/world_data.hpp>
#include <src/util/conversions.hpp>

namespace copper_server::build_in_plugins::base {
    struct chunk_speed_data {
        int64_t x;
        int64_t z;
        std::chrono::milliseconds tick_time;
    };

    void end_chunk_speed_report_collecting(
        const std::filesystem::path& path,
        const std::string& world_name,
        list_array<list_array<chunk_speed_data>>&& collected_data,
        base_objects::client_data_holder executor
    ) {
        try {
            enbt::fixed_array enbt_collected_data;
            enbt_collected_data.reserve(collected_data.size());
            for (const auto& tick : collected_data) {
                enbt::fixed_array enbt_tick;
                enbt_tick.reserve(tick.size());
                for (const auto& chunk : tick)
                    enbt_tick.push_back(enbt::fixed_array{chunk.x, chunk.z, chunk.tick_time.count()});
                enbt_collected_data.push_back(std::move(enbt_tick));
            }
            auto current_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
            std::tm* timeinfo = std::localtime(&current_time);
            auto report_path = path
                               / "reports"
                               / "chunk_tick_speed"
                               / (std::to_string(timeinfo->tm_year + 1900)
                                  + "_" + std::to_string(timeinfo->tm_mon + 1)
                                  + "_" + std::to_string(timeinfo->tm_mday)
                                  + "_" + std::to_string(timeinfo->tm_hour)
                                  + "_" + std::to_string(timeinfo->tm_min)
                                  + "_" + std::to_string(timeinfo->tm_sec)
                                  + ".senbt");
            std::filesystem::create_directories(report_path.parent_path());
            fast_task::files::async_iofstream report_file(
                report_path,
                fast_task::files::open_mode::write,
                fast_task::files::on_open_action::create_new,
                fast_task::files::_sync_flags{}
            );

            if (!report_file.is_open()) {
                base_objects::chat message("Failed to save chunk tick speed report for world: " + world_name + " to: " + report_path.string());
                message.set_color("red");
                *executor << api::packets::client_bound::play::system_chat{.content = message};
            }
            report_file << senbt::serialize(enbt_collected_data);
            //enbt::io_helper::write_token(report_file, enbt_collected_data);

            base_objects::chat message("Chunk tick speed report for world: " + world_name + " saved to: " + report_path.string());
            message.set_color("green");
            *executor << api::packets::client_bound::play::system_chat{.content = message};
        }

        catch (const std::exception& e) {
            api::log::error("World", "Failed to save chunk speed report: " + std::string(e.what()));
        }
    }

    void start_chunk_speed_report_collecting(storage::world_data& world, const base_objects::command_context& context) {
        world.profiling.chunk_speedometer_callback
            = [start_time = std::chrono::high_resolution_clock::now(),
               collected_data = list_array<list_array<chunk_speed_data>>(),
               current_tick = list_array<chunk_speed_data>(),
               executor = api::players::get_player(context.executor)](storage::world_data& world, int64_t chunk_x, int64_t chunk_z, std::chrono::milliseconds tick_time) mutable {
                  if (chunk_x == INT64_MAX && chunk_z == INT64_MAX && tick_time.count() == 0) {
                      collected_data.push_back(std::move(current_tick));
                      if (start_time + std::chrono::milliseconds(10000) < std::chrono::high_resolution_clock::now()) {
                          base_objects::chat message("Saving chunk tick speed report for world: " + world.world_name);
                          message.set_color("green");
                          *executor << api::packets::client_bound::play::system_chat{.content = message};
                          fast_task::task::run([path = world.get_path(), world_name = world.world_name, collected_data = std::move(collected_data), executor]() mutable {
                              end_chunk_speed_report_collecting(path, world_name, std::move(collected_data), executor);
                          });
                          world.profiling.chunk_speedometer_callback = nullptr;
                      }
                  } else
                      current_tick.emplace_back(chunk_speed_data(chunk_x, chunk_z, tick_time));
              };
    }

    void world_tps_profiling(storage::world_data& world) {
        api::log::info("World", "Profiling TPS for world " + world.world_name + ": " + std::to_string(world.profiling.tps_for_world));
    }

    void world_slow_chunk_notify_profiling(storage::world_data& world, int64_t chunk_x, int64_t chunk_z, std::chrono::milliseconds tick_time) {
        std::string message = "Got slow chunk tick for world " + world.world_name + " chunk " + std::to_string(chunk_x) + " " + std::to_string(chunk_z) + " in " + std::to_string(tick_time.count()) + "ms";
        api::log::warn("World", message);
    }

    void slow_world_notify_profiling(storage::world_data& world, std::chrono::milliseconds tick_time) {
        std::string message = "Can't keep up with world " + world.world_name + " in " + std::to_string(tick_time.count()) + "ms";
        api::log::warn("World", message);
    }

    int32_t world_seed_or_java_hash(const std::string& str) {
        auto is_num = str.find_first_not_of("0123456789") == std::string::npos;
        if (is_num)
            return std::stoi(str);
        else {
            int32_t hash = 0, multiplier = 1;
            for (ptrdiff_t i = str.size() - 1; i >= 0; i--) {
                hash += str[i] * multiplier;
                multiplier = (multiplier << 5) - multiplier;
            }
            return hash;
        }
    }

    class WorldManagementPlugin : public plugin_auto_register<"base/world_manager", WorldManagementPlugin> {
        storage::worlds_data worlds_storage;
        std::shared_ptr<fast_task::task> world_ticking;

        static void add_world_id_suggestion(base_objects::command_browser& browser) {
            browser.set_suggestion_callback([](const std::string& current, base_objects::command_context&) {
                auto suggestions = api::world::request_ids().convert<std::string>([](int32_t id) {
                    return std::to_string(id);
                });
                if (current.empty())
                    return suggestions;
                else {
                    return suggestions.where([&current](const std::string& suggestion) { return suggestion.starts_with(current); });
                }
            });
        }

        static void add_world_name_suggestion(base_objects::command_browser& browser) {
            browser.set_suggestion_callback([](const std::string& current, base_objects::command_context&) {
                auto suggestions = api::world::request_names();
                if (current.empty())
                    return suggestions;
                else {
                    return suggestions.where([&current](const std::string& suggestion) { return suggestion.starts_with(current); });
                }
            });
        }

    public:
        WorldManagementPlugin()
            : worlds_storage(api::configuration::get().server.get_worlds_path()) {}

        ~WorldManagementPlugin() noexcept {}

        void on_initialization(const plugin_registration_ptr&) override {
            api::world::register_worlds_data(worlds_storage);
        }

        void on_load(const plugin_registration_ptr&) override {
            api::log::info("World", "loading worlds...");
            register_event(worlds_storage.on_world_loaded, [](int32_t id) {
                api::log::debug("World", "world id " + std::to_string(id) + " loaded.");
                api::world::get(id, [&](storage::world_data& world) {
                    world.profiling.enable_world_profiling = true;
                    world.profiling.slow_chunk_tick_callback = world_slow_chunk_notify_profiling;
                    world.profiling.slow_world_tick_callback = slow_world_notify_profiling;
                });
                return false;
            });
            register_event(worlds_storage.on_world_unloaded, [](int32_t id) {
                api::log::debug("World", "world id " + std::to_string(id) + " unloaded.");
                return false;
            });

            std::list<std::shared_ptr<fast_task::task>> tasks;
            for (auto& it : api::configuration::get().allowed_dimensions) {
                api::world::pre_load_world(it, [&](storage::world_data& world) {
                    auto& cfg_world = api::configuration::get().world;
                    world.set_seed(world_seed_or_java_hash(cfg_world.seed));
                    world.light_processor_id = "default";
                    if (world.world_name == "end") {
                        world.generator_id = "end";
                        world.set_world_type("minecraft:the_end");
                    } else if (world.world_name == "nether") {
                        world.generator_id = "nether";
                        world.set_world_type("minecraft:the_nether");
                    } else {
                        world.set_world_type(cfg_world.type);
                        world.generator_id = cfg_world.generator_type;
                        for (auto& [name, value] : cfg_world.generator_settings)
                            world.world_generator_data[name] = value;
                    }
                    api::log::info("World", it + " initialized.");
                });

                tasks.push_back(std::make_shared<fast_task::task>([it]() {
                    bool complete = false;
                    api::world::get(it, [&](storage::world_data& world) {
                        plugin_management.ecs_registrators({}, world.entity_tick_scheduler);//TODO add filter
                    });
                    while (!complete) {
                        size_t target = 0;
                        size_t loaded = 0;
                        api::world::get(it, [&](storage::world_data& world) {
                            target = world.profiling.chunk_target_to_load;
                            loaded = world.profiling.chunk_total_loaded;
                        });
                        if (target <= loaded && target) {
                            api::log::info("World", "world " + it + " loaded.");
                            complete = true;
                        } else {
                            api::log::info("World", "world " + it + " loading... " + std::to_string(loaded) + " / " + std::to_string(target));
                            fast_task::this_task::sleep_for(std::chrono::milliseconds(500));
                        }
                    }
                }));
            }
            if (worlds_storage.base_world_id == -1 && !api::configuration::get().allowed_dimensions.empty()) {
                api::log::info("World", "Base world not set, setting to first world.");
                if (auto id = worlds_storage.get_id("overworld"); id != -1) {
                    worlds_storage.base_world_id = id;
                } else
                    worlds_storage.base_world_id = worlds_storage.get_list()[0];
            }
            api::log::debug("World", "installing ticking task...");
            world_ticking = std::make_shared<fast_task::task>([this]() {
                api::log::debug("World", "ticking task installed");
                std::chrono::high_resolution_clock::time_point tick_next_awoke = std::chrono::high_resolution_clock::now();
                while (!worlds_storage.ticks_per_second)
                    fast_task::this_task::sleep_for(std::chrono::milliseconds(1000));
                constexpr auto second = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::seconds(1));

                auto sleep_time = std::chrono::high_resolution_clock::now() - tick_next_awoke;
                while (true) {
                    if (fast_task::this_task::is_cancellation_requested())
                        break;
                    auto current_time = std::chrono::high_resolution_clock::now();
                    try {
                        worlds_storage.apply_tick(current_time);
                    } catch (const std::exception& e) {
                        api::log::error("World", "Error ticking world: " + std::string(e.what()));
                    } catch (...) {
                        api::log::error("World", "Error ticking world. Undefined exception.");
                    }
                    tick_next_awoke = std::chrono::high_resolution_clock::now();
                    auto to_tick = tick_next_awoke - current_time;
                    const auto tick_time = second / worlds_storage.ticks_per_second;

                    if (to_tick < tick_time + sleep_time) {
                        tick_next_awoke += tick_time - to_tick - sleep_time;
                        fast_task::this_task::sleep_for(tick_time - to_tick - sleep_time);
                        sleep_time = std::chrono::high_resolution_clock::now() - tick_next_awoke;
                    }
                }
            });
            fast_task::scheduler::start(world_ticking);
            fast_task::task::await_multiple(tasks, false, true);
        }

        void on_post_unload(const plugin_registration_ptr&) override {
            api::log::info("World", "saving worlds...");
            if (world_ticking) {
                world_ticking->await_notify_cancel();
                world_ticking = nullptr;
            }

            worlds_storage.locked([&](storage::worlds_data& worlds) {
                worlds.for_each_world([&](int32_t _, storage::world_data& world) {
                    api::log::info("World", "saving world " + world.world_name + "...");
                    world.save();
                    world.save_chunks(true);
                    world.await_save_chunks();
                    api::log::info("World", "world " + world.world_name + " saved.");
                });
                worlds.unload_all();
            });
        }

        void on_fault_unload(const plugin_registration_ptr&) override {
            api::log::info("World", "saving worlds...");
            worlds_storage.locked([&](storage::worlds_data& worlds) {
                world_ticking->notify_cancel();
                worlds.for_each_world([&](int32_t _, storage::world_data& world) {
                    api::log::info("World", "saving world " + world.world_name + "...");
                    world.save();
                    world.save_chunks(false);
                    world.await_save_chunks();
                    api::log::info("World", "world " + world.world_name + " saved.");
                });
            });
        }

        using predicate = base_objects::parser;
        using pred_double = base_objects::parsers::_double;
        using cmd_pred_double = base_objects::parsers::command::_double;
        using pred_int = base_objects::parsers::_integer;
        using cmd_pred_int = base_objects::parsers::command::_integer;
        using pred_long = base_objects::parsers::_long;
        using cmd_pred_long = base_objects::parsers::command::_long;
        using pred_string = base_objects::parsers::string;
        using cmd_pred_string = base_objects::parsers::command::string;

        using pred_nbt_compound_tag = base_objects::parsers::nbt_compound_tag;
        using cmd_pred_nbt_compound_tag = base_objects::parsers::command::nbt_compound_tag;

        using pred_block_pos = base_objects::parsers::block_pos;
        using cmd_pred_block_pos = base_objects::parsers::command::block_pos;

        using pred_angle = base_objects::parsers::angle;
        using cmd_pred_angle = base_objects::parsers::command::angle;

        using pred_block = base_objects::parsers::block_state;
        using cmd_pred_block = base_objects::parsers::command::block_state;

        using pred_entity = base_objects::parsers::entity;
        using cmd_pred_entity = base_objects::parsers::command::entity;

        using pred_resource_key = base_objects::parsers::resource_key;
        using cmd_pred_resource_key = base_objects::parsers::command::resource_key;

        static base_objects::full_block_data extract_block(const pred_block& block_data) {
            auto& static_block_data = base_objects::block::get_block(block_data.block_id);
            auto states = static_block_data.assigned_states_to_properties->left.at(static_block_data.default_state);
            for (auto& [state, value] : block_data.states) {
                states.at(state) = value;
            }
            base_objects::block block(static_block_data.assigned_states_to_properties->right.at(states));
            if (block_data.data_tags.is_end())
                return block;
            else
                return base_objects::block_entity(block, block_data.data_tags);
        }

        static void OnCommandsLoad_world_pre_gen(base_objects::command_browser& worlds) {
            auto pre_gen = worlds.add_child("pre_gen");
            auto world_id = pre_gen.add_child("world_id", cmd_pred_int());
            auto world_name = pre_gen.add_child("world_name", cmd_pred_string());

            auto x0 = world_id.add_child("x0", cmd_pred_long());
            world_name.add_child(x0);
            auto z0 = x0.add_child("z0", cmd_pred_long());
            auto x1 = z0.add_child("x1", cmd_pred_long());
            auto z1 = x1.add_child("z1", cmd_pred_long());

            z1.set_callback("command.world.pre_gen", [](const list_array<predicate>& args, base_objects::command_context&) {
                base_objects::cubic_bounds_chunk bound(
                    std::get<pred_long>(args[1]).value,
                    std::get<pred_long>(args[2]).value,
                    std::get<pred_long>(args[3]).value,
                    std::get<pred_long>(args[4]).value
                );
                size_t count = 0;
                std::visit(
                    [&](auto&& arg) {
                        using T = std::decay_t<decltype(arg)>;
                        if constexpr (std::is_same_v<T, pred_int> || std::is_same_v<T, pred_string>) {
                            api::world::get(arg.value, [&](storage::world_data& world) {
                                bound.enum_points([&](auto x, auto z) {
                                    world.request_chunk_gen(x, z);
                                    ++count;
                                });
                            });
                        }
                    },
                    args[0]
                );
                return count;
            });

            add_world_id_suggestion(world_id);
            add_world_name_suggestion(world_name);
        }

        static void OnCommandsLoad_world_create(base_objects::command_browser& worlds) {
            auto create = worlds.add_child("create");
            auto world_name = create.add_child("world_name", cmd_pred_string());
            auto settings = world_name.add_child("settings", cmd_pred_nbt_compound_tag());
            settings
                .set_callback("command.world.create", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    const std::string& name = std::get<pred_string>(args[0]).value;
                    auto& settings = std::get<pred_nbt_compound_tag>(args[1]).nbt;
                    if (api::world::exists(name)) {
                        base_objects::chat message("Failed to create world, world with this name already exists: " + name);
                        message.set_color("red");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return false;
                    } else {
                        api::world::create(name, [&](storage::world_data& world) {
                            world.load(settings);
                        });
                        base_objects::chat message("World created: " + name);
                        message.set_color("green");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return true;
                    }
                });
            world_name.set_callback("command.world.create", [](const list_array<predicate>& args, base_objects::command_context& context) {
                const std::string& name = std::get<pred_string>(args[0]).value;
                if (api::world::exists(name)) {
                    base_objects::chat message("Failed to create world, world with this name already exists: " + name);
                    message.set_color("red");
                    context.executor << api::packets::client_bound::play::system_chat{.content = message};
                    return false;
                } else {
                    api::world::create(name);
                    base_objects::chat message("World created: " + name);
                    message.set_color("green");
                    context.executor << api::packets::client_bound::play::system_chat{.content = message};
                    return true;
                }
            });
        }

        static void OnCommandsLoad_world_remove(base_objects::command_browser& worlds) {
            auto remove = worlds.add_child("remove");
            auto world_id = remove.add_child("world_id", cmd_pred_int());
            auto world_name = remove.add_child("world_name", cmd_pred_string());

            world_id
                .set_callback("command.world.remove", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    int32_t id = std::get<pred_int>(args[0]).value;
                    if (!api::world::exists(id)) {
                        base_objects::chat message("Failed to set world id, world with this id not set: " + std::to_string(id));
                        message.set_color("red");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return false;
                    } else
                        api::world::remove(id);
                    return true;
                });
            world_name
                .set_callback("command.world.remove", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    const std::string& id = std::get<pred_string>(args[0]).value;
                    auto actual_id = api::world::resolve_id(id);
                    if (!api::world::exists(actual_id)) {
                        base_objects::chat message("Failed to set world id, world with this id not set: " + id);
                        message.set_color("red");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return false;
                    } else
                        api::world::remove(actual_id);
                    return true;
                });
            add_world_id_suggestion(world_id);
            add_world_name_suggestion(world_name);
        }

        static void OnCommandsLoad_world_base(base_objects::command_browser& worlds) {
            auto base = worlds.add_child("base");
            {
                auto set = base.add_child("set");
                auto world_id = set.add_child("world_id", cmd_pred_int());
                auto world_name = set.add_child("world_name", cmd_pred_string());
                world_id
                    .set_callback("command.world.base.set", [](const list_array<predicate>& args, base_objects::command_context& context) {
                        int32_t id = std::get<pred_int>(args[0]).value;
                        if (!api::world::exists(id)) {
                            base_objects::chat message("Failed to set world id, world with this id not set: " + std::to_string(id));
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            return false;
                        } else
                            api::world::set_default_world_id(id);
                        return true;
                    });
                world_name
                    .set_callback("command.world.base.set", [](const list_array<predicate>& args, base_objects::command_context& context) {
                        const std::string id = std::get<pred_string>(args[0]).value;
                        auto actual_id = api::world::resolve_id(id);
                        if (!api::world::exists(actual_id)) {
                            base_objects::chat message("Failed to set world id, world with this id not set: " + id);
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            return false;
                        } else
                            api::world::set_default_world_id(actual_id);
                        return true;
                    });
                add_world_id_suggestion(world_id);
                add_world_name_suggestion(world_name);
            }
            base.set_callback("command.world.base", [](const list_array<predicate>&, base_objects::command_context& context) {
                if (api::world::get_default_world_id() == -1) {
                    context.executor << api::packets::client_bound::play::system_chat{.content = "Base world not set."};

                    return false;
                } else
                    api::world::get(api::world::get_default_world_id(), [&context](auto& world) {
                        context.executor << api::packets::client_bound::play::system_chat{.content = "Base world is: " + world.world_name + " (" + std::to_string(api::world::get_default_world_id()) + ")"};
                    });
                return true;
            });
        }

        static void OnCommandsLoad_world_load(base_objects::command_browser& worlds) {

            auto load = worlds.add_child("load");
            auto world_id = load.add_child("world_id", cmd_pred_int());
            auto world_name = load.add_child("world_name", cmd_pred_string());
            world_id
                .set_callback("command.world.load", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    int32_t id = std::get<pred_int>(args[0]).value;
                    if (!api::world::exists(id)) {
                        base_objects::chat message("Failed to load world, world with this id not set: " + std::to_string(id));
                        message.set_color("red");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return false;
                    } else {
                        api::world::get(id, [](auto&) {});
                        base_objects::chat message("World loaded: " + api::world::resolve_name(id));
                        message.set_color("green");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return true;
                    }
                });
            world_name
                .set_callback("command.world.load", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    const std::string id = std::get<pred_string>(args[0]).value;
                    auto actual_id = api::world::resolve_id(id);
                    if (!api::world::exists(actual_id)) {
                        base_objects::chat message("Failed to load world, world with this id not set: " + id);
                        message.set_color("red");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return false;
                    } else {
                        api::world::get(actual_id, [](auto&) {});
                        base_objects::chat message("World loaded: " + api::world::resolve_name(actual_id));
                        message.set_color("green");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return true;
                    }
                });
            add_world_id_suggestion(world_id);
            add_world_name_suggestion(world_name);
        }

        static void OnCommandsLoad_world_save(base_objects::command_browser& worlds) {
            auto save = worlds.add_child("save");
            auto world_id = save.add_child("world_id", cmd_pred_int());
            auto world_name = save.add_child("world_name", cmd_pred_string());
            world_id
                .set_callback("command.world.save", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    int32_t id = std::get<pred_int>(args[0]).value;
                    if (!api::world::exists(id)) {
                        base_objects::chat message("Failed to save world, world with this id not set: " + std::to_string(id));
                        message.set_color("red");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return false;
                    } else {
                        api::world::save(id);
                        base_objects::chat message("World saved: " + api::world::resolve_name(id));
                        message.set_color("green");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return true;
                    }
                });
            world_name
                .set_callback("command.world.save", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    const std::string id = std::get<pred_string>(args[0]).value;
                    auto actual_id = api::world::resolve_id(id);
                    if (!api::world::exists(actual_id)) {
                        base_objects::chat message("Failed to save world, world with this id not set: " + id);
                        message.set_color("red");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return false;
                    } else {
                        api::world::save(actual_id);
                        base_objects::chat message("World saved: " + api::world::resolve_name(actual_id));
                        message.set_color("green");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return true;
                    }
                });
            add_world_id_suggestion(world_id);
            add_world_name_suggestion(world_name);
        }

        static void OnCommandsLoad_world_save_all(base_objects::command_browser& worlds) {
            auto save_all = worlds.add_child("save_all");
            save_all.set_callback("command.world.save_all", [](const list_array<predicate>&, base_objects::command_context& context) {
                api::world::save_all();
                base_objects::chat message("All worlds saved.");
                message.set_color("green");
                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                return true;
            });
        }

        static void OnCommandsLoad_world_list(base_objects::command_browser& worlds) {
            auto list = worlds.add_child("list");
            list.set_callback("command.world.list", [](const list_array<predicate>&, base_objects::command_context& context) {
                std::string message = "Worlds: ";
                api::world::for_each_world([&message](int32_t _, storage::world_data& world) {
                    message += world.world_name + ", ";
                });
                message.erase(message.size() - 2);
                message[message.size() - 1] = '.';
                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                return true;
            });
        }

        static void OnCommandsLoad_world_unload(base_objects::command_browser& worlds) {
            auto unload = worlds.add_child("unload");
            auto world_id = unload.add_child("world_id", cmd_pred_int());
            auto world_name = unload.add_child("world_name", cmd_pred_string());
            world_id
                .set_callback("command.world.unload", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    int32_t id = std::get<pred_int>(args[0]).value;
                    if (!api::world::exists(id)) {
                        base_objects::chat message("Failed to unload world, world with this id not set: " + std::to_string(id));
                        message.set_color("red");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return false;
                    } else {
                        api::world::save_and_unload(id);
                        base_objects::chat message("World unloaded: " + api::world::resolve_name(id));
                        message.set_color("green");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return true;
                    }
                });
            world_name
                .set_callback("command.world.unload", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    const std::string id = std::get<pred_string>(args[0]).value;
                    auto actual_id = api::world::resolve_id(id);
                    if (!api::world::exists(actual_id)) {
                        base_objects::chat message("Failed to unload world, world with this id not set: " + id);
                        message.set_color("red");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return false;
                    } else {
                        api::world::save_and_unload(actual_id);
                        base_objects::chat message("World unloaded: " + api::world::resolve_name(actual_id));
                        message.set_color("green");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return false;
                    }
                });
            add_world_id_suggestion(world_id);
            add_world_name_suggestion(world_name);
        }

        static void OnCommandsLoad_world_unload_all(base_objects::command_browser& worlds) {
            auto unload_all = worlds.add_child("unload_all");
            unload_all.set_callback("command.world.unload_all", [](const list_array<predicate>&, base_objects::command_context& context) {
                api::world::save_and_unload_all();
                base_objects::chat message("All worlds unloaded.");
                message.set_color("green");
                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                return true;
            });
        }

        static void OnCommandsLoad_world_get_id(base_objects::command_browser& worlds) {
            auto get_id = worlds.add_child("get_id");
            auto world_name = get_id.add_child("world_name", cmd_pred_string());
            world_name
                .set_callback("command.world.get_id", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    const std::string id = std::get<pred_string>(args[0]).value;
                    auto actual_id = api::world::resolve_id(id);
                    if (actual_id == -1) {
                        base_objects::chat message("Failed to get world id, world with this name not set: " + id);
                        message.set_color("red");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return false;
                    } else {
                        base_objects::chat message("World id: " + std::to_string(actual_id));
                        message.set_color("green");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return true;
                    }
                });
            add_world_name_suggestion(world_name);
        }

        static void OnCommandsLoad_world_get_name(base_objects::command_browser& worlds) {
            auto get_name = worlds.add_child("get_name");
            auto world_id = get_name.add_child("world_id", cmd_pred_int());
            world_id
                .set_callback("command.world.get_name", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    int32_t id = std::get<pred_int>(args[0]).value;
                    if (!api::world::exists(id)) {
                        base_objects::chat message("Failed to get world name, world with this id not set: " + std::to_string(id));
                        message.set_color("red");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return false;
                    } else {
                        base_objects::chat message("World name: " + api::world::resolve_name(id));
                        message.set_color("green");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        return true;
                    }
                });
            add_world_id_suggestion(world_id);
        }

        static void OnCommandsLoad_world_set_block(base_objects::command_browser& worlds) {
            auto setblock = worlds.add_child("setblock");
            auto world = setblock.add_child("world_id", cmd_pred_int());
            auto world_name = setblock.add_child("world_name", cmd_pred_string());
            auto position = world.add_child("xyz", cmd_pred_block_pos());
            world_name.add_child(position);
            auto block = position.add_child("block", cmd_pred_block());
            auto replace = block.add_child("replace");
            auto destroy = block.add_child("destroy");
            auto keep = block.add_child("keep");

            auto command = [](const list_array<predicate>& args, base_objects::command_context& context) {
                auto& world = args[0];
                auto pos = std::get<pred_block_pos>(args[1]);
                auto block = extract_block(std::get<pred_block>(args[2]));

                if (pos.x_relative)
                    pos.x += (int32_t)context.other_data["x"];
                if (pos.y_relative)
                    pos.y += (int32_t)context.other_data["y"];
                if (pos.z_relative)
                    pos.z += (int32_t)context.other_data["z"];
                bool res = false;

                std::visit(
                    [&world, &pos, &res, &block](auto&& arg) {
                        using T = std::decay_t<decltype(arg)>;
                        if constexpr (std::is_same_v<T, pred_int>) {
                            api::world::get(std::get<pred_int>(world).value, [pos, &res, &block](storage::world_data& world) {
                                world.set_block(std::move(block), pos.x, pos.y, pos.z, storage::block_set_mode::replace);
                                res = true;
                            });
                        } else {
                            api::world::get(std::get<pred_string>(world).value, [pos, &res, &block](storage::world_data& world) {
                                world.set_block(std::move(block), pos.x, pos.y, pos.z, storage::block_set_mode::replace);
                                res = true;
                            });
                        }
                    },
                    world
                );
                return res;
            };

            replace.set_callback("command.world.setblock", command);
            block.set_callback("command.world.setblock", command);

            destroy.set_callback("command.world.setblock", [](const list_array<predicate>& args, base_objects::command_context& context) {
                auto& world = args[0];
                auto pos = std::get<pred_block_pos>(args[1]);
                auto block = extract_block(std::get<pred_block>(args[2]));

                if (pos.x_relative)
                    pos.x += (int32_t)context.other_data["x"];
                if (pos.y_relative)
                    pos.y += (int32_t)context.other_data["y"];
                if (pos.z_relative)
                    pos.z += (int32_t)context.other_data["z"];
                bool res = false;

                std::visit(
                    [&world, &pos, &res, &block](auto&& arg) {
                        using T = std::decay_t<decltype(arg)>;
                        if constexpr (std::is_same_v<T, pred_int>) {
                            api::world::get(std::get<pred_int>(world).value, [pos, &res, &block](storage::world_data& world) {
                                world.set_block(std::move(block), pos.x, pos.y, pos.z, storage::block_set_mode::destroy);
                                res = true;
                            });
                        } else {
                            api::world::get(std::get<pred_string>(world).value, [pos, &res, &block](storage::world_data& world) {
                                world.set_block(std::move(block), pos.x, pos.y, pos.z, storage::block_set_mode::destroy);
                                res = true;
                            });
                        }
                    },
                    world
                );
                return res;
            });

            keep.set_callback("command.world.setblock", [](const list_array<predicate>& args, base_objects::command_context& context) {
                auto& world = args[0];
                auto pos = std::get<pred_block_pos>(args[1]);
                auto block = extract_block(std::get<pred_block>(args[2]));

                if (pos.x_relative)
                    pos.x += (int32_t)context.other_data["x"];
                if (pos.y_relative)
                    pos.y += (int32_t)context.other_data["y"];
                if (pos.z_relative)
                    pos.z += (int32_t)context.other_data["z"];
                bool res = false;

                std::visit(
                    [&world, &pos, &res, &block](auto&& arg) {
                        using T = std::decay_t<decltype(arg)>;
                        if constexpr (std::is_same_v<T, pred_int>) {
                            api::world::get(std::get<pred_int>(world).value, [pos, &res, &block](storage::world_data& world) {
                                world.set_block(std::move(block), pos.x, pos.y, pos.z, storage::block_set_mode::keep);
                                res = true;
                            });
                        } else {
                            api::world::get(std::get<pred_string>(world).value, [pos, &res, &block](storage::world_data& world) {
                                world.set_block(std::move(block), pos.x, pos.y, pos.z, storage::block_set_mode::keep);
                                res = true;
                            });
                        }
                    },
                    world
                );
                return res;
            });


            add_world_id_suggestion(world);
            add_world_name_suggestion(world_name);
        }

        static void OnCommandsLoad_world_profile_report(base_objects::command_browser& world_id_, base_objects::command_browser& world_name_) {
            auto world_id = world_id_.add_child("report");
            auto world_name = world_name_.add_child("report");
            {
                auto chunk_tick_speed = world_id.add_child("chunk_tick_speed");
                world_name.add_child(chunk_tick_speed);
                chunk_tick_speed.set_callback("command.world.profile.report.chunk_tick_speed", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    bool res = false;

                    std::visit(
                        [&](auto&& arg) {
                            using T = std::decay_t<decltype(arg)>;
                            if constexpr (std::is_same_v<T, pred_int> || std::is_same_v<T, pred_string>) {
                                api::world::get(arg.value, [&](storage::world_data& world) {
                                    if (world.profiling.chunk_speedometer_callback) {
                                        base_objects::chat message("Failed to report chunks tick speed, chunk speed profiling already enabled for world: " + world.world_name);
                                        message.set_color("red");
                                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                                        return;
                                    } else {
                                        start_chunk_speed_report_collecting(world, context);
                                        base_objects::chat message("Collecting chunks tick speed report for world: " + world.world_name);
                                        message.set_color("green");
                                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                                        res = true;
                                    }
                                });
                            }
                        },
                        args[0]
                    );
                    return res;
                });
            }
        }

        static void OnCommandsLoad_world_profile_enable(base_objects::command_browser& world_id_, base_objects::command_browser& world_name_) {
            auto world_id = world_id_.add_child("enable");
            auto world_name = world_name_.add_child("enable");
            world_id
                .add_child("tps")
                .set_callback("command.world.profile.enable.tps", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_int>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        if (world.profiling.enable_world_profiling && world.profiling.got_tps_update) {
                            base_objects::chat message("Profiling already enabled for world: " + world.world_name);
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            return;
                        }
                        world.profiling.enable_world_profiling = true;
                        world.profiling.got_tps_update = world_tps_profiling;
                        base_objects::chat message("Profiling enabled for world: " + world.world_name);
                        message.set_color("green");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        res = true;
                    });
                    return res;
                });

            world_name
                .add_child("tps")
                .set_callback("command.world.profile.enable.tps", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_string>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        if (world.profiling.enable_world_profiling && world.profiling.got_tps_update) {
                            base_objects::chat message("TPS profiling already enabled for world: " + world.world_name);
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            return;
                        }
                        world.profiling.enable_world_profiling = true;
                        world.profiling.got_tps_update = world_tps_profiling;
                        base_objects::chat message("TPS profiling enabled for world: " + world.world_name);
                        message.set_color("green");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        res = true;
                    });
                    return res;
                });

            world_id
                .add_child("slow_chunk")
                .set_callback("command.world.profile.enable.slow_chunk", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_int>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        if (world.profiling.enable_world_profiling && world.profiling.slow_chunk_tick_callback) {
                            base_objects::chat message("Chunk profiling already enabled for world: " + world.world_name);
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            return;
                        }
                        world.profiling.enable_world_profiling = true;
                        world.profiling.slow_chunk_tick_callback = world_slow_chunk_notify_profiling;
                        base_objects::chat message("Chunk profiling enabled for world: " + world.world_name);
                        message.set_color("green");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        res = true;
                    });
                    return res;
                });

            world_name
                .add_child("slow_chunk")
                .set_callback("command.world.profile.enable.slow_chunk", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_string>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        if (world.profiling.enable_world_profiling && world.profiling.slow_chunk_tick_callback) {
                            base_objects::chat message("Chunk profiling already enabled for world: " + world.world_name);
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            return;
                        }
                        world.profiling.enable_world_profiling = true;
                        world.profiling.slow_chunk_tick_callback = world_slow_chunk_notify_profiling;
                        base_objects::chat message("Chunk profiling enabled for world: " + world.world_name);
                        message.set_color("green");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        res = true;
                    });
                    return res;
                });

            world_id
                .add_child("slow_world")
                .set_callback("command.world.profile.enable.slow_world", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_int>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        if (world.profiling.enable_world_profiling && world.profiling.slow_world_tick_callback) {
                            base_objects::chat message("World profiling already enabled for world: " + world.world_name);
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            return;
                        }
                        world.profiling.enable_world_profiling = true;
                        world.profiling.slow_world_tick_callback = slow_world_notify_profiling;
                        base_objects::chat message("World profiling enabled for world: " + world.world_name);
                        message.set_color("green");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        res = true;
                    });
                    return res;
                });

            world_name
                .add_child("slow_world")
                .set_callback("command.world.profile.enable.slow_world", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_string>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        if (world.profiling.enable_world_profiling && world.profiling.slow_world_tick_callback) {
                            base_objects::chat message("World profiling already enabled for world: " + world.world_name);
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            return;
                        }
                        world.profiling.enable_world_profiling = true;
                        world.profiling.slow_world_tick_callback = slow_world_notify_profiling;
                        base_objects::chat message("World profiling enabled for world: " + world.world_name);
                        message.set_color("green");
                        context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        res = true;
                    });
                    return res;
                });
        }

        static void OnCommandsLoad_world_profile_disable(base_objects::command_browser& world_id_, base_objects::command_browser& world_name_) {
            auto world_id = world_id_.add_child("disable");
            auto world_name = world_name_.add_child("disable");


            world_id
                .set_callback("command.world.profile.disable", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_int>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        if (world.profiling.enable_world_profiling) {
                            world.profiling.enable_world_profiling = false;
                            world.profiling.slow_chunk_tick_callback = nullptr;
                            world.profiling.slow_world_tick_callback = nullptr;
                            world.profiling.got_tps_update = nullptr;
                            world.profiling.chunk_speedometer_callback = nullptr;

                            base_objects::chat message("Profiling disabled for world: " + world.world_name);
                            message.set_color("green");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            res = true;
                        } else {
                            base_objects::chat message("TPS profiling already disabled for world: " + world.world_name);
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        }
                    });
                    return res;
                });

            world_name
                .set_callback("command.world.profile.disable", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_string>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        if (world.profiling.enable_world_profiling) {
                            world.profiling.enable_world_profiling = false;
                            world.profiling.slow_chunk_tick_callback = nullptr;
                            world.profiling.slow_world_tick_callback = nullptr;
                            world.profiling.got_tps_update = nullptr;

                            base_objects::chat message("Profiling disabled for world: " + world.world_name);
                            message.set_color("green");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            res = true;
                        } else {
                            base_objects::chat message("TPS profiling already disabled for world: " + world.world_name);
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        }
                    });
                    return res;
                });


            world_id
                .add_child("tps")
                .set_callback("command.world.profile.disable.tps", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_int>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        if (world.profiling.enable_world_profiling) {
                            if (*world.profiling.got_tps_update.target<void (*)(storage::world_data&)>() == world_tps_profiling) {
                                world.profiling.got_tps_update = nullptr;
                                if (!world.profiling.slow_chunk_tick_callback && !world.profiling.slow_world_tick_callback)
                                    world.profiling.enable_world_profiling = false;

                                base_objects::chat message("TPS profiling disabled for world: " + world.world_name);
                                message.set_color("green");
                                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                                res = true;
                            } else {
                                base_objects::chat message("TPS profiling enabled by another plugin for world: " + world.world_name);
                                message.set_color("red");
                                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            }
                        } else {
                            base_objects::chat message("TPS profiling already disabled for world: " + world.world_name);
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        }
                    });
                    return res;
                });

            world_name
                .add_child("tps")
                .set_callback("command.world.profile.disable.tps", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_string>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        if (world.profiling.enable_world_profiling) {
                            if (*world.profiling.got_tps_update.target<void (*)(storage::world_data&)>() == world_tps_profiling) {
                                world.profiling.got_tps_update = nullptr;
                                if (!world.profiling.slow_chunk_tick_callback && !world.profiling.slow_world_tick_callback)
                                    world.profiling.enable_world_profiling = false;
                                base_objects::chat message("TPS profiling disabled for world: " + world.world_name);
                                message.set_color("green");
                                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                                res = true;
                            } else {
                                base_objects::chat message("TPS profiling enabled by another plugin for world: " + world.world_name);
                                message.set_color("red");
                                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            }
                        } else {
                            base_objects::chat message("TPS profiling already disabled for world: " + world.world_name);
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        }
                    });
                    return res;
                });

            world_id
                .add_child("slow_chunk")
                .set_callback("command.world.profile.disable.slow_chunk", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_int>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        if (world.profiling.enable_world_profiling) {
                            if (*world.profiling.slow_chunk_tick_callback.target<void (*)(storage::world_data&, int64_t, int64_t, std::chrono::milliseconds)>() == world_slow_chunk_notify_profiling) {
                                world.profiling.slow_chunk_tick_callback = nullptr;
                                base_objects::chat message("Slow chunk profiling enabled for world: " + world.world_name);
                                message.set_color("green");
                                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                                res = true;
                            } else {
                                base_objects::chat message("Slow chunk profiling enabled by another plugin for world: " + world.world_name);
                                message.set_color("red");
                                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            }
                        } else {
                            base_objects::chat message("Slow chunk profiling already disabled for world: " + world.world_name);
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        }
                    });
                    return res;
                });

            world_name
                .add_child("slow_chunk")
                .set_callback("command.world.profile.disable.slow_chunk", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_string>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        if (world.profiling.enable_world_profiling) {
                            if (*world.profiling.slow_chunk_tick_callback.target<void (*)(storage::world_data&, int64_t, int64_t, std::chrono::milliseconds)>() == world_slow_chunk_notify_profiling) {
                                world.profiling.slow_chunk_tick_callback = nullptr;
                                base_objects::chat message("Slow chunk profiling enabled for world: " + world.world_name);
                                message.set_color("green");
                                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            } else {
                                base_objects::chat message("Slow chunk profiling enabled by another plugin for world: " + world.world_name);
                                message.set_color("red");
                                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            }
                        } else {
                            base_objects::chat message("Slow chunk profiling already disabled for world: " + world.world_name);
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        }
                    });
                    return res;
                });

            world_id
                .add_child("slow_world")
                .set_callback("command.world.profile.disable.slow_world", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_int>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        if (world.profiling.enable_world_profiling) {
                            if (*world.profiling.slow_world_tick_callback.target<void (*)(storage::world_data&, std::chrono::milliseconds)>() == slow_world_notify_profiling) {
                                world.profiling.slow_world_tick_callback = nullptr;
                                base_objects::chat message("Slow chunk profiling enabled for world: " + world.world_name);
                                message.set_color("green");
                                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                                res = true;
                            } else {
                                base_objects::chat message("Slow chunk profiling enabled by another plugin for world: " + world.world_name);
                                message.set_color("red");
                                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            }
                        } else {
                            base_objects::chat message("Slow chunk profiling already disabled for world: " + world.world_name);
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        }
                    });
                    return res;
                });

            world_name
                .add_child("slow_world")
                .set_callback("command.world.profile.disable.slow_world", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_string>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        if (world.profiling.enable_world_profiling) {
                            if (*world.profiling.slow_world_tick_callback.target<void (*)(storage::world_data&, std::chrono::milliseconds)>() == slow_world_notify_profiling) {
                                world.profiling.slow_world_tick_callback = nullptr;
                                base_objects::chat message("Slow chunk profiling enabled for world: " + world.world_name);
                                message.set_color("green");
                                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                                res = true;
                            } else {
                                base_objects::chat message("Slow chunk profiling enabled by another plugin for world: " + world.world_name);
                                message.set_color("red");
                                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                            }
                        } else {
                            base_objects::chat message("Slow chunk profiling already disabled for world: " + world.world_name);
                            message.set_color("red");
                            context.executor << api::packets::client_bound::play::system_chat{.content = message};
                        }
                    });
                    return res;
                });
        }

        static void OnCommandsLoad_world_profile_config(base_objects::command_browser& world_id_, base_objects::command_browser& world_name_) {
            auto config_id = world_id_.add_child("config");
            auto config_name = world_name_.add_child("config");

            config_id
                .add_child("slow_world_threshold")
                .set_callback("command.world.profile.config.slow_world_threshold.get", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_int>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        context.executor << api::packets::client_bound::play::system_chat{.content = "Slow world threshold: " + std::to_string(world.profiling.slow_world_tick_callback_threshold)};
                        res = true;
                    });
                    return res;
                })
                .add_child("value", cmd_pred_double())
                .set_callback("command.world.profile.config.slow_world_threshold.set", [](const list_array<predicate>& args, base_objects::command_context&) {
                    auto& world_id = std::get<pred_int>(args[0]);
                    auto& value = std::get<pred_double>(args[1]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        world.profiling.slow_world_tick_callback_threshold = value.value;
                        res = true;
                    });
                    return res;
                });

            config_name
                .add_child("slow_world_threshold")
                .set_callback("command.world.profile.config.slow_world_threshold.get", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_string>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        context.executor << api::packets::client_bound::play::system_chat{.content = "Slow world threshold: " + std::to_string(world.profiling.slow_world_tick_callback_threshold)};
                        res = true;
                    });
                    return res;
                })
                .add_child("value", cmd_pred_double())
                .set_callback("command.world.profile.config.slow_world_tick_threshold.set", [](const list_array<predicate>& args, base_objects::command_context&) {
                    auto& world_name = std::get<pred_string>(args[0]);
                    auto& value = std::get<pred_double>(args[1]);
                    bool res = false;

                    api::world::get(world_name.value, [&](storage::world_data& world) {
                        world.profiling.slow_world_tick_callback_threshold = value.value;
                        res = true;
                    });
                    return res;
                });

            config_id
                .add_child("slow_chunk_threshold")
                .set_callback("command.world.profile.config.slow_world_threshold.get", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_int>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        context.executor << api::packets::client_bound::play::system_chat{.content = "Slow chunk threshold: " + std::to_string(world.profiling.slow_chunk_tick_callback_threshold)};
                        res = true;
                    });
                    return res;
                })
                .add_child("value", cmd_pred_double())
                .set_callback("command.world.profile.config.slow_chunk_threshold.set", [](const list_array<predicate>& args, base_objects::command_context&) {
                    auto& world_id = std::get<pred_int>(args[0]);
                    auto& value = std::get<pred_double>(args[1]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        world.profiling.slow_chunk_tick_callback_threshold = value.value;
                        res = true;
                    });
                    return res;
                });

            config_name
                .add_child("slow_chunk_threshold")
                .set_callback("command.world.profile.config.slow_world_threshold.get", [](const list_array<predicate>& args, base_objects::command_context& context) {
                    auto& world_id = std::get<pred_string>(args[0]);
                    bool res = false;

                    api::world::get(world_id.value, [&](storage::world_data& world) {
                        context.executor << api::packets::client_bound::play::system_chat{.content = "Slow chunk threshold: " + std::to_string(world.profiling.slow_chunk_tick_callback_threshold)};
                        res = true;
                    });
                    return res;
                })
                .add_child("value", cmd_pred_double())
                .set_callback("command.world.profile.config.slow_chunk_threshold.set", [](const list_array<predicate>& args, base_objects::command_context&) {
                    auto& world_name = std::get<pred_string>(args[0]);
                    auto& value = std::get<pred_double>(args[1]);
                    bool res = false;

                    api::world::get(world_name.value, [&](storage::world_data& world) {
                        world.profiling.slow_chunk_tick_callback_threshold = value.value;
                        res = true;
                    });
                    return res;
                });
        }

        static void OnCommandsLoad_world_profile(base_objects::command_browser& worlds) {
            auto profile = worlds.add_child("profile");
            auto world_id_ = profile.add_child("world_id", cmd_pred_int());
            auto world_name_ = profile.add_child("world_name", cmd_pred_string());
            add_world_id_suggestion(world_id_);
            add_world_name_suggestion(world_name_);
            OnCommandsLoad_world_profile_config(world_id_, world_name_);
            OnCommandsLoad_world_profile_disable(world_id_, world_name_);
            OnCommandsLoad_world_profile_enable(world_id_, world_name_);
            OnCommandsLoad_world_profile_report(world_id_, world_name_);
        }

        static void OnCommandsLoad_world_chunks_loaded(base_objects::command_browser& worlds) {
            auto chunks_loaded = worlds.add_child("chunks_loaded");
            chunks_loaded.set_callback("command.chunks_loaded", [](const list_array<predicate>&, base_objects::command_context& context) {
                base_objects::chat message("Chunks loaded: " + std::to_string(api::world::loaded_chunks_count()));
                message.set_color("green");
                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                return true;
            });
            auto world_id = chunks_loaded.add_child("world_id", cmd_pred_int());
            world_id.set_callback("command.chunks_loaded", [](const list_array<predicate>& args, base_objects::command_context& context) {
                auto world_id = std::get<pred_int>(args[0]).value;
                base_objects::chat message("Chunks loaded: " + std::to_string(api::world::loaded_chunks_count(world_id)));
                message.set_color("green");
                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                return true;
            });

            auto world_name = chunks_loaded.add_child("world_name", cmd_pred_string());
            world_name.set_callback("command.chunks_loaded", [](const list_array<predicate>& args, base_objects::command_context& context) {
                auto& world_name = std::get<pred_string>(args[0]).value;
                base_objects::chat message("Chunks loaded: " + std::to_string(api::world::loaded_chunks_count(world_name)));
                message.set_color("green");
                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                return true;
            });
            add_world_id_suggestion(world_id);
            add_world_name_suggestion(world_name);
        }

        static void OnCommandsLoad_world(base_objects::command_root_browser& browser) {
            auto worlds = browser.add_child("worlds");
            OnCommandsLoad_world_pre_gen(worlds);
            OnCommandsLoad_world_create(worlds);
            OnCommandsLoad_world_remove(worlds);
            OnCommandsLoad_world_base(worlds);
            OnCommandsLoad_world_load(worlds);
            OnCommandsLoad_world_save(worlds);
            OnCommandsLoad_world_save_all(worlds);
            OnCommandsLoad_world_list(worlds);
            OnCommandsLoad_world_unload(worlds);
            OnCommandsLoad_world_unload_all(worlds);
            OnCommandsLoad_world_get_id(worlds);
            OnCommandsLoad_world_get_name(worlds);
            OnCommandsLoad_world_set_block(worlds);
            OnCommandsLoad_world_profile(worlds);
            OnCommandsLoad_world_chunks_loaded(worlds);
        }

        static void OnCommandsLoad_setblock(base_objects::command_root_browser& browser) {
            auto setblock = browser.add_child("setblock");
            auto position = setblock.add_child("xyz", cmd_pred_block_pos());
            auto block = position.add_child("block", cmd_pred_block());
            auto replace = block.add_child("replace");
            auto destroy = block.add_child("destroy");
            auto keep = block.add_child("keep");


            auto command
                = [](const list_array<predicate>& args, base_objects::command_context& context) {
                      auto pos = std::get<pred_block_pos>(args[0]);
                      auto block = extract_block(std::get<pred_block>(args[1]));

                      if (pos.x_relative)
                          pos.x += (int32_t)context.other_data["x"];
                      if (pos.y_relative)
                          pos.y += (int32_t)context.other_data["y"];
                      if (pos.z_relative)
                          pos.z += (int32_t)context.other_data["z"];

                      api::world::get((int32_t)context.other_data["world_id"], [pos, &block](storage::world_data& world) {
                          world.set_block(std::move(block), pos.x, pos.y, pos.z, storage::block_set_mode::replace);
                      });
                      return true;
                  };
            replace.set_callback("command.setblock", command);
            block.set_callback("command.setblock", command);

            destroy.set_callback("command.setblock", [](const list_array<predicate>& args, base_objects::command_context& context) {
                auto pos = std::get<pred_block_pos>(args[0]);
                auto block = extract_block(std::get<pred_block>(args[1]));

                if (pos.x_relative)
                    pos.x += (int32_t)context.other_data["x"];
                if (pos.y_relative)
                    pos.y += (int32_t)context.other_data["y"];
                if (pos.z_relative)
                    pos.z += (int32_t)context.other_data["z"];

                api::world::get((int32_t)context.other_data["world_id"], [pos, &block](storage::world_data& world) {
                    world.set_block(std::move(block), pos.x, pos.y, pos.z, storage::block_set_mode::destroy);
                });
                return true;
            });

            keep.set_callback("command.setblock", [](const list_array<predicate>& args, base_objects::command_context& context) {
                auto pos = std::get<pred_block_pos>(args[0]);
                auto block = extract_block(std::get<pred_block>(args[1]));

                if (pos.x_relative)
                    pos.x += (int32_t)context.other_data["x"];
                if (pos.y_relative)
                    pos.y += (int32_t)context.other_data["y"];
                if (pos.z_relative)
                    pos.z += (int32_t)context.other_data["z"];

                api::world::get((int32_t)context.other_data["world_id"], [pos, &block](storage::world_data& world) {
                    world.set_block(std::move(block), pos.x, pos.y, pos.z, storage::block_set_mode::keep);
                });
                return true;
            });
        }

        static void OnCommandsLoad_setbiome(base_objects::command_root_browser& browser) {
            auto setblock = browser.add_child("setbiome");
            auto position = setblock.add_child("xyz", cmd_pred_block_pos());
            auto biome = position.add_child("biome", cmd_pred_resource_key{.suggestion_registry = "minecraft:worldgen/biome"});
            auto box_ = position.add_child("xyz_2", cmd_pred_block_pos());
            auto biome_box = box_.add_child("biome", cmd_pred_resource_key{.suggestion_registry = "minecraft:worldgen/biome"});

            biome.set_callback("command.setbiome", [](const list_array<predicate>& args, base_objects::command_context& context) {
                auto pos = std::get<pred_block_pos>(args[0]);
                auto biome = std::get<pred_resource_key>(args[1]).value;
                api::id::worldgen__biome real_biome = biome;

                if (pos.x_relative)
                    pos.x += (int32_t)context.other_data["x"];
                if (pos.y_relative)
                    pos.y += (int32_t)context.other_data["y"];
                if (pos.z_relative)
                    pos.z += (int32_t)context.other_data["z"];

                api::world::get((int32_t)context.other_data["world_id"], [pos, real_biome](storage::world_data& world) {
                    world.set_biome(pos.x, pos.y, pos.z, real_biome);
                });
                return true;
            });
            biome_box.set_callback("command.setbiome", [](const list_array<predicate>& args, base_objects::command_context& context) {
                auto pos = std::get<pred_block_pos>(args[0]);
                auto pos_2 = std::get<pred_block_pos>(args[1]);
                auto biome = std::get<pred_resource_key>(args[2]).value;
                api::id::worldgen__biome real_biome = biome;

                if (pos.x_relative)
                    pos.x += (int32_t)context.other_data["x"];
                if (pos.y_relative)
                    pos.y += (int32_t)context.other_data["y"];
                if (pos.z_relative)
                    pos.z += (int32_t)context.other_data["z"];

                if (pos_2.x_relative)
                    pos_2.x += (int32_t)context.other_data["x"];
                if (pos_2.y_relative)
                    pos_2.y += (int32_t)context.other_data["y"];
                if (pos_2.z_relative)
                    pos_2.z += (int32_t)context.other_data["z"];
                base_objects::cubic_bounds_block bounds{pos.x, pos.y, pos.z, pos_2.x, pos_2.y, pos_2.z};

                api::world::get((int32_t)context.other_data["world_id"], [bounds, real_biome](storage::world_data& world) {
                    world.set_biome_range(bounds, {(int32_t)real_biome});
                });
                return true;
            });
        }

        static void OnCommandsLoad_getworldspawn(base_objects::command_root_browser& browser) {
            auto getworldspawn = browser.add_child("getworldspawn");
            getworldspawn.set_callback("command.getworldspawn", [](const list_array<predicate>&, base_objects::command_context& context) {
                base_objects::chat message("World spawn: x: " + std::to_string(api::configuration::get().world.spawn.x) + " y: " + std::to_string(api::configuration::get().world.spawn.y) + " z: " + std::to_string(api::configuration::get().world.spawn.z) + " yaw: " + std::to_string(api::configuration::get().world.spawn.yaw));
                message.set_color("green");
                context.executor << api::packets::client_bound::play::system_chat{.content = message};
                return true;
            });
        }

        static void OnCommandsLoad_worldborder(base_objects::command_root_browser& browser) {
            //auto worldborder = browser.add_child("worldborder"); //TODO
            //worldborder.add_child("add").add_child("distance", )
        }

        void on_commands_load(const plugin_registration_ptr& _, base_objects::command_root_browser& browser) override {
            OnCommandsLoad_world(browser);
            OnCommandsLoad_setblock(browser);
            OnCommandsLoad_setbiome(browser);
            OnCommandsLoad_getworldspawn(browser);
        }

        void on_config_reload(const plugin_registration_ptr& _) override {
            api::world::for_each_world([speed = api::configuration::get().world.load_speed](auto id, auto& world) {
                world.update_load_limit(speed);
            });
        }
    };
}
