...
namespace copper_server::build_in_plugins::network::tcp::client_handler {
    base_objects::network::response handle_login::IdleActions() {
        ...
        if (auto packets = session->shared_data().getPendingPackets(); packets.size()) {
            ...
            if (
                ...
                return api::packets::login::encryptionRequest(session->shared_data(), "", login_data.verify_token);
            }
            ...
            if (auto threshold = api::configuration::get().protocol.compression_threshold; threshold != -1 && login_data.login_check == 1) {
                base_objects::network::response response = api::packets::login::setCompression(session->shared_data(), threshold);
            ...
            while (!login_data.plugins_query.empty()) {
                ...
                auto plugin_response = self.second->OnLoginStart(self.second, self.first, session->shared_data_ref());
                auto res = std::visit(
                    [&](auto& it) {
    ...
    base_objects::network::response handle_login::too_large_packet() {
        return api::packets::login::kick(session->shared_data(), "Packet too large");
    }

    base_objects::network::response handle_login::exception(const std::exception& ex) {
        return api::packets::login::kick(session->shared_data(), "Internal server error: " + std::string(ex.what()) + "\nPlease report this to the server owner!");
    }

    base_objects::network::response handle_login::unexpected_exception() {
        return api::packets::login::kick(session->shared_data(), "Internal server error\nPlease report this to the server owner!");
    }
    ...
    handle_login::handle_login(api::network::tcp::session* session)
        ...
        auto& login_data = *(session->shared_data().packets_state.login_data = base_objects::SharedClientData::packets_state_t::state_login{});
    ...
    handle_login::~handle_login() {
        session->shared_data().packets_state.login_data = nullptr;
    }

    base_objects::network::tcp::client* handle_login::define_ourself(api::network::tcp::session* sock) {
        return nullptr;
    }
}