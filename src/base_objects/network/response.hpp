...
namespace copper_server::base_objects::network {
    struct response_item {
        ...
        response_item();
        response_item(const list_array<uint8_t>& data, int32_t compression_threshold = -1, bool apply_compression = false);
        response_item(list_array<uint8_t>&& data, int32_t compression_threshold = -1, bool apply_compression = false);
        ...
        //appends response_item data to current response_item, but does not affect the flags
        void write_in(response_item&& data);
        void write_in(const response_item& data);
    ...
    struct response {
        response();
        response(const response_item& move);
        response(response_item&& move);
        ...
        static response answer(const list_array<response_item>& data, size_t valid_till = 0);
        static response answer(list_array<response_item>&& data, size_t valid_till = 0);
        static response empty();
        static response disconnect();
        static response disconnect(const list_array<response_item>& data, size_t valid_till = 0);
        static response disconnect(list_array<response_item>&& data, size_t valid_till = 0);
    ...
    private:
        response(const list_array<response_item>& response_bytes, size_t valid_till, bool disconnect = false, bool disconnect_after_send = false);
        response(list_array<response_item>&& response_bytes, size_t valid_till, bool disconnect = false, bool disconnect_after_send = false);
    };
