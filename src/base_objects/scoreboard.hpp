...
namespace copper_server::base_objects {
    class objective {
        struct event_auto_cleanup_t {
            events::base_event* event_obj;
            events::event_register_id id;
    ...
    public:
        events::event<std::string> on_score_change;
        events::event<std::string> on_score_remove;
        ...
        template <class T>
        size_t register_event(base_objects::events::event<T>& event_ref, base_objects::events::event<T>::function&& fn) {
            return protected_vals.set([&](protected_vals_t& vals) {
                vals.cleanup_list[vals.next_id++] = {event_ref.join(base_objects::events::priority::avg, false, fn), event_ref, base_objects::events::priority::avg, false};
                return vals.next_id - 1;
            });
        }

        template <class T>
        size_t register_event(base_objects::events::event<T>& event_ref, base_objects::events::priority priority, base_objects::events::event<T>::function&& fn) {
            return protected_vals.set([&](protected_vals_t& vals) {
        ...
        template <class T>
        size_t register_event(base_objects::events::event<T>& event_ref, base_objects::events::priority priority, bool async_mode, base_objects::events::event<T>::function&& fn) {
            return protected_vals.set([&](protected_vals_t& vals) {
