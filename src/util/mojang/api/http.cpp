...
namespace copper_server::util::mojang::api::http {
    ...
    std::string get(const std::string& address, const std::string& query, uint16_t port, uint8_t max_redirects) {
        return request("GET", address, query, port, max_redirects);
    }

    std::string post(const std::string& address, const std::string& query, uint16_t port, uint8_t max_redirects) {
