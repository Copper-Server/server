...
namespace copper_server::util::mojang::api {
    void hash256::two_complement(hashed_array& data) {
        bool carry = true;
    ...
    void hash256::to_hex(hashed_array data, char* hex) {
    ...
    void hash256::update(const void* data, size_t size) {
        EVP_DigestUpdate(ctx, data, size);
    }

    uint64_t hash256::to_part_hash() {
        hashed_array md;
