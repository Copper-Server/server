...
struct TCPsession {
    ...
    uint64_t id = id_gen++;
    TCPsession(baip::tcp::socket&& s, TCPclient* client_handler, uint64_t& set_timeout) : sock(std::move(s)), timeout(set_timeout){
        chandler = client_handler->DefineOurself(sock);
    }
    TCPsession(TCPsession&& move) noexcept : sock(std::move(move.sock)), timeout(move.timeout) {
            ...
            sock.async_wait(sock.wait_read, [this](boost::system::error_code ec) {
                if (checked(ec, "on_wait")) {
                    if (size_t aviable = sock.available()) {
                        ...
                        async_read(sock, boost::asio::dynamic_buffer(data, 2097154),
                            [this](boost::system::error_code ec, size_t xfr) { on_request(ec, xfr); });
                }
            });
        }
    }
...
class TCPserver
{
    ...
    void Worker() {
        ...
        TCPacceptor.async_accept(session->sock, [this, session](const boost::system::error_code& error) {
            ...
            if (!buf->size()) {
                async_read(session->sock, boost::asio::dynamic_buffer(*buf, max_accept_buffer),
                    [this, session, buf](boost::system::error_code ec, size_t xfr) {  AsyncWork(session, buf); });
            }
            else AsyncWork(session,buf);
        });
    }
