...
namespace mcCore {
	...
	class WorldClusters {
		...
		inline void stateChnged(UPDATE_BLOCK block_pos, Block bock_state) {
		...
		static std::list<UPDATE_BLOCK> _UpdateBlockSelect(Block::TickAnswer to_be_calc, UPDATE_BLOCK tick_pos) {
			...
			case Block::TickAnswer::PosMode::as_tunels:
			{
				std::list<UPDATE_BLOCK> res;
				for (auto x : RangeInt(tick_pos.x + to_be_calc.X, tick_pos.x + to_be_calc.x))
		...
		static std::list<UPDATE_BLOCK> _UpdateBlockSelectMoveFreded(Block::TickAnswer to_be_calc, UPDATE_BLOCK tick_pos) {
			...
			if (to_be_calc.move_x) {
				...
				tmp.remove_if(
					to_be_calc.move_x > 0 ?
					std::function([max_x](UPDATE_BLOCK& check) { return max_x < check.x; }) :
					std::function([max_x](UPDATE_BLOCK& check) { return max_x > check.x; })
				);
			...
			if (to_be_calc.move_y) {
				...
				tmp.remove_if(
					to_be_calc.move_y > 0 ?
					std::function([max_y](UPDATE_BLOCK& check) { return max_y < check.y; }) :
					std::function([max_y](UPDATE_BLOCK& check) { return max_y > check.y; })
				);
				res.insert(res.end(), tmp.begin(), tmp.end());
			}
			if (to_be_calc.move_z) {
				...
				tmp.remove_if(
					to_be_calc.move_z > 0 ?
					std::function([max_z](UPDATE_BLOCK& check) { return max_z < check.z; }) :
					std::function([max_z](UPDATE_BLOCK& check) { return max_z > check.z; })
				);
				res.insert(res.end(), tmp.begin(), tmp.end());
			}
		...
		void _UpdateBlockUpdateNow(size_t& uppdated_now, std::list<UPDATE_BLOCK>& on_updaing_blocks, std::list<UPDATE_BLOCK>& add_blocks) {
			if (uppdated_now + add_blocks.size() <= max_one_tick_update_now) {
				on_updaing_blocks.insert(on_updaing_blocks.end(), add_blocks.begin(), add_blocks.end());
				uppdated_now += add_blocks.size();
			}
			else {
				to_be_updqateed_mut.lock();
				to_be_updated.insert(to_be_updated.end(), add_blocks.begin(), add_blocks.end());
				to_be_updqateed_mut.unlock();
			}
		}
		void UpdateBlock(UPDATE_BLOCK ub) {
			...
			while (tmp.empty()) {
				...
				case Block::TickAnswer::Behavior::check_can_move_and_move_blocks:
					*global_reactor += [this, tanswer, it]() {
				...
				case Block::TickAnswer::Behavior::check_can_move_and_move_blocks_then_add_new_blocks:
					*global_reactor += [this, tanswer, it]() {
						...
						if (CalculateCanMoveBlocks(temp, tanswer.move_x, tanswer.move_y, tanswer.move_z)) {
							MoveBlocks(temp, tanswer.move_x, tanswer.move_y, tanswer.move_z);
							setBlocks(_UpdateBlockSelectMoveFreded(tanswer, it), *tanswer.block);
						}
				...
				case Block::TickAnswer::Behavior::update_now:
				{
					...
					_UpdateBlockUpdateNow(uppdated_now, tmp, insert_value);
					break;
				}
		...
		void _EventNotifyHandler(BlockEvent data, WorldClusterTalker* handler) {
			...
			case BlockEvent::Reason::nbt_changed:
				int64_t chuster_x_dif = handler->cluster_x - data.x / 24;
				int64_t chuster_z_dif = handler->cluster_z - data.z / 24;
				if (+chuster_x_dif <= handler->cluster_handle_distance)
					if (+chuster_z_dif <= +handler->cluster_handle_distance)
	...
	public:
		...
		static WorldClusters* getWorldByID(uint32_t world_id) { return nullptr; }
		...
		WorldClusters() {
			if (global_reactor == nullptr)
				throw std::exception("For world, global reactor will be defined");
		}
		~WorldClusters() {
